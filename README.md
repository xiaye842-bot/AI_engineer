...
任务数据存储在 Electron `userData` 目录下的 `engineering-tasks.json`。文件采用版本化结构并通过临时文件替换写入；无法解析的文件会保留为带时间戳的 `.corrupt-*` 备份，然后创建新的任务库。归档不会删除任务内容，永久删除会移除完整任务包并要求二次确认。

工程任务包当前包含任务模式、任务类型、任务元数据、六阶段状态、关键需求字段、对话消息、证据、风险、设计决策和审计轨迹。工程流程模式必须按顺序流转，且当前阶段结论不少于 10 个字符并经工程师确认；常规快速模式不执行阶段流转。

## 非文本文件接入

Pi `AgentSession.prompt()` 支持 `ImageContent[]` 附件，Pi 的只读工具也能识别常见图片 MIME 类型。但当前桌面端消息 IPC 仍只传输文本，因此图片识别尚未接入 UI。建议后续增加：

- 图片拖放/选择、缩略图、大小限制和 MIME 校验
- 主进程读取文件并转换为 base64 `ImageContent`
- 根据模型目录的 `input` 能力阻止向纯文本模型发送图片
- 将原始文件、哈希、解析结果和人工结论写入证据链
- PDF、Office、日志、波形等文件优先使用专用解析器生成结构化数据，再交给模型分析

图标、截图、原理图和测试波形需要支持视觉输入的多模态模型。PDF、Office、二进制日志、音视频和专用测试文件不能只依赖模型“直接看”，应配置对应解析 Skill/Tool；重要工程结论仍需规则校验和人工确认。
